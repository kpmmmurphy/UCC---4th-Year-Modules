...
The main question I then asked myself was: how can safety and security be improved within the home, through the leveraging of modern hardware and software components? I kept this question in mind throughout the entire project lifecycle.
...
An embedded sensor system and Android  application which supplies the user with live environmental data and security services. \\\\
...
The final software architecture was composed of three main components, with the addition of a peripheral sensing system, which was introduced during the development phase. The components include:
...
\noindent
Implemented in Python, a very flexible and easy to use dynamically typed language. Its succinct nature and dynamic typing aid rapid application development. The community support behind the language also greatly encouraged its adoption for this section of the system. \\\\ To implement, I used the PyCharm IDE. This provided real time code validation, automated code generation and syntactical checking.\\\\ All direct interaction with sensors is handled via a C++ shared object library, which is invoked via Python Ctypes. The shared object library was developed natively on the Raspberry Pi using the g++ GNU C++ Compiler. This method ensures maximum performence when probing sensors. 
...
To control sensors connected to the Raspberry Pi, an implementation using the open source WiringPi C/C++ general purpose input/output(GPIO) library was developed. This library provides basic interaction mechanisms for both analog and digital sensors. The community support for this library is substantial, and has become the de facto standard library for embedded Raspberry Pi projects. \\\\ The C++ sensor implementation developed was compiled into a shared object library, which is callable via the Python foreign function library Ctypes. The command to compile the C++ share object library can be seen in Listing~\ref{lst:c++_compile} below. 

\begin{lstlisting}[caption={C++ sensor implementation shared object library compile command},label={lst:c++_compile}]
sudo g++ -Wall -fPIC -v -shared *.h *.cpp -o lib_Sensors.so -lwiringPi
\end{lstlisting}

\noindent
Using the shared object library involves defining a Python wrapper, which is displayed in Listing~\ref{lst:python_wrapper} below.

\begin{lstlisting}[caption={Python Ctypes wrapper},label={lst:python_wrapper}]
//Setup method arguments for ctypes
self.__lib.MQ7_newInstance.argtypes = [ctypes.c_char_p, ctypes.c_int]
    
//Setup method return types for ctypes
...
//Create new sensor object instance from shared object library 
self.obj = self.__lib.MQ7_newInstance(self.__name,self.__adcChannelNo)

//Query the sensor for its latest value
latestValue = self.__lib.MQ7_readValue(self.obj) 
\end{lstlisting}

\noindent
A limitation of the Raspberry Pi is that is does not provide any analog GPIO pins. To overcome this, an additional analog to digital converter(ADC) was added to the system\rq s circuitry. The MCP3008 ADC, an 8 channel 10-bit analog input,  was chosen due to its support in the WiringPi GPIO code library. This facilitated easy access to any analog sensor that needed to be included. \\\\

\begin{figure}[H]
\centering
\includegraphics[width=6cm, height=8cm]{"mcp3008".JPG}
...
The Javascript Object Notation(JSON) was utilised as the means for specifying the required state configuration of individual classes. The Configuration Manager, in Figure~\ref{fig:sms_high_level_arch}, was delegated the responsibility for administering the updating of the system\rq s configuration. Listing~\ref{lst:sms_json_config} below shows the default JSON configuration structure. It contains an object for each manager class present in the system, and an array of sensor objects with individual state values for each. \\\\
...
The system alerts in two ways. Proximity altering in the form of a physical buzzer present on the circuitry, and direct mobile device push notification alerting via the Google Cloud Messaging(GCM) Service.  \\\\
Each sensor maintains its own alerting threshold, and when reached it performs its defined behaviour. For example, when the carbon monoxide sensor\rq s threshold is reached, a push notification is sent, and the physical buzzer attached to the system\rq s circuitry is sounded. Additional to sensor threshold alerting, the system can be configured to be in a lockdown state, where in the event of the infrared motion detector detecting activity, the on board camera is activated and a push notification is sent. \\\\
To send a push notification, a registration process must first take place between the Android application and the GCM service. This process is described in the Android Application implementation Push Notification Section~\ref{sssec:android_push_notifications}. Resulting from this registration is a unique device identification number, which must be persistently stored. In this implementation, the registration ids were stored by the RESTful API, and retrieved prior to sending each push notification. \\\\
The registration id is sent along with a JSON formatted payload via a HTTP request to the GCM service, which handles delivering the payload to the respective device. An example push notification payload can be seen below in Listing~\ref{lst:sms_pn}.
\begin{center}
...
\end{table}
\noindent
This method facilitates case based reactive error handling, and allows for easier debugging as the problem is automatically narrowed down to one of three possibilities. A timestamp is also included for general debugging purposes. Depending on where the request originated, further fields may be included. \\\\
...
The API maintains 3 MySQL database tables, identical to those running natively on the Sensor Management System but with the addition of the Push Notification Details table. The original design separating the sensor values table would limit the memory storage required, but also increase the underlying functionality needed to manage the additional tables. The final implementation accomplishes the same functionality, but at the loss of storage optimisation. \
\begin{figure}[H]
\centering
...
\begin{figure}[H]
\centering
\includegraphics[width=12cm, height=15cm]{"opening_screen_shot".png}
...
In order to maximise performance and reduce code within the application, no persistent database storage is used. Instead, when possible computation is delegated to either the server side API, or the Sensor Management System. This is to ensure that the application runs seamlessly, and is only required to perform small computational tasks. This allows for better management of limited resources, allowing them to be designated to the application\rq s user interface components, ensuring fluid operation. \\\\
...
\noindent
Underneath the sensor tiles is a text view which displays the date and time of the sensor readings, this can be seen in Figure~\ref{fig:android_sensor_timestamp}. In the right corner is the button which toggles between the physical sensing systems.
\begin{figure}[H]
\centering
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth, height=12cm]{"context_help".png}
...
\noindent
An example of the API method definition for sensor values is displayed in Listing~\ref{lst:java_api_call} below.
...
\begin{lstlisting}[caption={Java method API call for requesting latest sensor readings},label={lst:java_api_call}]
...
\begin{enumerate}
  ...
  \item Perform in app registration of the device to the GCM server, which generates a unique device identification number. This must be stored persistently by the RESTful API and is used to target notifications to the specific device
...
\newpage
\subsubsection{System Configuration}
As described in Section~\ref{sssec:sms_sys_config}, Sensor Management System Configuration, the JSON object notation is used to configure the system. The Control fragment within the application provides the user with an interface for altering configurations of the Sensor Management System. It was implemented as a form, with each section titled by the underlying system component it configures. 
...
\noindent
The Sensor Manager section, in Figure~\ref{fig:android_config_1}, handles the time interval, in seconds, at which the sensor data is written out to persistent storage. This affects the granularity of the data being provided to the charts in the Stats fragment.  A priority can also be set on the thread which executes the sensor data storage. \\\\
The Alert Manager section, in Figure~\ref{fig:android_config_1}, provides the ability to turn on or off the camera, alert buzzer, push notifications, and lockdown mode. When the lockdown mode is active, if motion is detected, the camera will be activated and a push notification will be sent to the user. Switching from the default of image capture to video capture is also facilitated. 
...
The Access Point Manager section, in Figure~\ref{fig:android_config_2}, sets the rate at which sensor values are sent to the application when directly communicating. \\\\
...
\newpage
\section{Conclusions}
Overall I think the project succeeded. The main goals I wanted to achieve with the system from the very beginning were remote sensor readings, configuration and alerting. These were the things I focused on implementing with a high degree of attention, basing the entire system architecture predominantly on them. By taking development objectives in small increments I was able to prototype a system that was fully operational in less than 6 months. Some shortcuts were taken, and little protected variation was included in my implementation. If I was to continue working on the project, a large amount of functionality would have to be recoded to improve extensibility, and efficiency. It\rq s important to remember that the system is a prototype, and with the Raspberry Pi\rq s limited resources, it\rq s hard to differentiate resource limitations, and the system\rq s limitation. In terms of hardware, I found the power and durability of the Raspberry Pi to be extraordinary. The board I developed the system on was 2 years old, and not once did it show signs of fault. 
\\\\
With a large amount of technologies incorporated in the project, being able to manage and unify each one in an extensible way really challenged me. The Git code versioning system was invaluable, and without it the project would simply not have been possible. 
\\\\
...
The following wiring diagrams are created using the Fritzing simulation tool~\footnote{http://fritzing.org/home/}.  
